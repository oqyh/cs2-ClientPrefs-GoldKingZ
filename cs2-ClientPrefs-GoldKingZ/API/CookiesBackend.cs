...
internal sealed class CookiesBackend<T> where T : class, new()
{
    ...
    public void Initialize()
    {
        try
        {
            ...
            _ = Task.Run(() =>
            {
                ...
                {
                    if (!_disposed) _debug($"SQLite LoadCache Failed ({_tableName}): {ex.Message}", true);
                }
            });
        }
        catch (Exception ex)
    ...
    public void EvictCached(ulong steamId) => _cache.TryRemove(steamId, out _);

    public List<(ulong steamId, string name, DateTime date, T payload)> SearchCached(Func<T, string, bool> match)
    {
        var results = new List<(ulong, string, DateTime, T)>();
        foreach (var kv in _cache)
        {
            try
            {
                if (match(kv.Value.payload, kv.Value.name))
                    results.Add((kv.Key, kv.Value.name, kv.Value.date, kv.Value.payload));
            }
            catch { }
        }
        return results;
