...
internal sealed class MySqlManager
{
    ...
    public void Release()
    {
        lock (_registryLock)
        {
            ...
            {
                try { using var tmp = new MySqlConnection(e.ConnectionString); MySqlConnection.ClearPool(tmp); } catch { }
            }
        }
    }
    ...
    public async Task<List<(string serverKey, MySqlConnection conn)>> GetAllConnectionsFastAsync(
        int timeout, Action<string, bool> debug)
    {
        if (_endpoints.Count == 0) return new List<(string, MySqlConnection)>();

        var tasks = _endpoints.Select(e => OpenEndpointFastAsync(e, timeout, debug)).ToArray();
        var results = await Task.WhenAll(tasks);
    ...
    private static async Task<(string key, MySqlConnection? conn)> OpenEndpointFastAsync(
        Endpoint e, int timeout, Action<string, bool> debug)
    {
        if (e.Healthy)
        {
            var conn = await TryOpenOnceAsync(e, timeout, debug);
            if (conn != null) return (e.Key, conn);
            e.Healthy = false;
            return (e.Key, null);
        }

        if (!e.Gate.Wait(0)) return (e.Key, null);
        try
        {
            var conn = await TryOpenOnceAsync(e, timeout, debug);
            if (conn != null)
