...
internal static class LoadNotifier
{
    ...
    private static Action<int, ulong>? _allLoadedSubscribers;

    public static void SubscribeAllLoaded(Action<int, ulong> cb)
    {
        lock (_lock) { _allLoadedSubscribers += cb; }
    }

    public static void UnsubscribeAllLoaded(Action<int, ulong> cb)
    {
        lock (_lock) { _allLoadedSubscribers -= cb; }
    }
    ...
    public static void RegisterPendingAll(int slot, ulong steamId)
    {
        lock (_lock)
        {
            var s = _slots.GetOrAdd(slot, _ => new SlotState { SteamId = steamId });
    ...
    public static void MarkReadyAll(int slot, ulong steamId)
    {
        Action<int, ulong>? subscribers = null;

        lock (_lock)
        {
            if (!_slots.TryGetValue(slot, out var s) || s.SteamId != steamId) return;

            if (s.AllPending > 0) s.AllPending--;

            if (s.AllPending == 0)
                subscribers = _allLoadedSubscribers;
        }
        ...
        if (subscribers != null)
        {
            Server.NextFrame(() =>
            {
                try { subscribers(slot, steamId); } catch { }
            });
        }
    }
