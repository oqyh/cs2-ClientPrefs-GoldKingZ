...
internal sealed class MySqlBackend<T> where T : class, new()
{
    ...
    private Task<List<(string serverKey, MySqlConnection conn)>> GetAllConnectionsFastAsync()
    {
        var mgr = _manager;
        if (_disposed || mgr == null)
            return Task.FromResult(new List<(string, MySqlConnection)>());

        return mgr.GetAllConnectionsFastAsync(_opts.PrefsAPI_MySqlConnectionTimeout, _debug);
    }
    ...
    {
        _debug($"MySQL [{serverKey}]: checking '{_tableName}' for column changes", false);
    ...
    public async Task<bool> SaveAsync(ulong steamId, string playerName, DateTime date, T payload, bool fast = false)
    {
        _info.FillNullStrings(payload);
        playerName ??= "";

        var conns = await (fast ? GetAllConnectionsFastAsync() : GetAllConnectionsAsync());
        if (conns.Count == 0) return false;
    ...
    public async Task<(DateTime date, T payload)?> LoadAsync(ulong steamId, bool fast = false)
    {
        var conns = await (fast ? GetAllConnectionsFastAsync() : GetAllConnectionsAsync());
    ...
    public async Task<List<(ulong steamId, string name, DateTime date, T payload)>> SearchAsync(string columnName, object value, bool fast = false)
    {
        var results = new Dictionary<ulong, (string name, DateTime date, T payload)>();
        ...
        foreach (var (key, conn) in conns)
        {
            try
            {
                ...
                await using var cmd = new MySqlCommand(
                    $"SELECT * FROM {_tableName} WHERE {columnName} = @v", conn);
                cmd.Parameters.AddWithValue("@v", value);

                await using var reader = await cmd.ExecuteReaderAsync();
                while (await reader.ReadAsync())
                {
                    var steamId = reader.GetUInt64(PrefsTypeInfo.PkColumn);
                    var date    = reader.GetDateTime(PrefsTypeInfo.DateColumn);
                    if (results.TryGetValue(steamId, out var ex) && ex.date >= date) continue;
                    ...
                    var name = "";
                    try { name = reader.GetString(PrefsTypeInfo.NameColumn); } catch { }
                    results[steamId] = (name, date, obj);
                }
            }
            catch (Exception ex)
            {
                _debug($"MySQL [{key}] SearchAsync error: {ex.Message}", true);
            }
            finally
            {
                try { await conn.DisposeAsync(); } catch { }
        ...
        return results.Select(kv => (kv.Key, kv.Value.name, kv.Value.date, kv.Value.payload)).ToList();
    }
